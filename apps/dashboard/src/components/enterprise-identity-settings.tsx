...
const COPY = {
  ...
  reauthenticate: msg({
    ...
    message: 'Sign in again with a passkey or equivalent strong method before changing this setting.',
  }),
  ownerOnly: msg({
  ...
  allowPassword: msg({ id: 'enterpriseIdentity.allowPassword', message: 'Allow password sessions' }),
  ...
  noDomains: msg({ id: 'enterpriseIdentity.noDomains', message: 'No company domain is configured.' }),
  ...
  requestCreated: msg({
    ...
    message: 'Request {requestId} expires at {expiresAt}. Give the ID to a different workspace owner.',
  }),
      ...
      <div className="grid items-start gap-5 xl:grid-cols-2">
        <Card className="shadow-none">
          ...
          <CardContent className="grid gap-4">
            ...
            />
            <form className="grid gap-3" noValidate onSubmit={(event) => void submitConnection(event)}>
              ...
              <FieldInput disabled={disabled} id="enterprise-issuer" label={_(COPY.issuer)} onChange={setIssuer} type="url" value={issuer} />
              <FieldInput disabled={disabled} id="enterprise-client-id" label={_(COPY.clientId)} onChange={setClientId} value={clientId} />
              <FieldLabel label={_(COPY.provisioning)}>
                <Select disabled={disabled} onValueChange={(value) => setProvisioningMode(value as EnterpriseProvisioningMode)} value={provisioningMode}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <ActionButton busy={busy === 'connection'} disabled={disabled} label={_(COPY.addConnection)} />
            </form>
          </CardContent>
        </Card>
        ...
        <Card className="shadow-none">
          ...
          <CardContent>
            ...
            >
              <BooleanControl checked={ssoRequired} disabled={disabled} label={_(COPY.requireSso)} onChange={setSsoRequired} />
              <BooleanControl checked={passwordAllowed} disabled={disabled} label={_(COPY.allowPassword)} onChange={setPasswordAllowed} />
              <FieldLabel label={_(COPY.minimumAssurance)}>
                <Select disabled={disabled} onValueChange={(value) => setMinimumAssurance(value as AuthAssuranceLevel)} value={minimumAssurance}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <FieldInput disabled={disabled} id="enterprise-break-glass-id" label={_(COPY.breakGlassId)} onChange={setBreakGlassId} value={breakGlassId} />
              <ActionButton busy={busy === 'policy'} disabled={disabled} label={_(COPY.savePolicy)} />
            </form>
          </CardContent>
        </Card>
        ...
        <Card className="shadow-none">
          ...
          <CardContent className="grid gap-4">
            {configuration.domains.length ? configuration.domains.map((entry) => (
              <div className="flex items-center justify-between gap-3 rounded-lg border border-border p-3" key={entry.id}>
            ...
            <form className="grid gap-3" noValidate onSubmit={(event) => void submitDomain(event)}>
              <ConnectionSelect connections={configuration.connections} disabled={disabled} label={_(COPY.connection)} onChange={setSelectedConnectionId} value={selectedConnectionId} />
              <FieldInput disabled={disabled} id="enterprise-domain" label={_(COPY.domain)} onChange={setDomain} value={domain} />
              <ActionButton busy={busy === 'domain'} disabled={disabled} label={_(COPY.startVerification)} />
            </form>
            {domainChallenge ? (
              <div className="grid gap-3 rounded-lg border border-border bg-muted/35 p-4">
                ...
                <ReadonlyValue label={_(COPY.recordValue)} value={domainChallenge.recordValue} />
                <Button disabled={Boolean(busy)} onClick={() => void runMutation('verify-domain', async () => {
                  const rawProof = domainChallenge.recordValue.replace(/^lodariq-domain-verification=/u, '');
                  await verifyEnterpriseDomain(workspaceId, domainChallenge.id, rawProof);
                  setDomainChallenge(null);
                })} type="button" variant="outline">
                  {busy === 'verify-domain' ? <LoaderCircle aria-hidden="true" className="animate-spin" /> : <ShieldCheck aria-hidden="true" />}
                  {_(COPY.verifyDomain)}
                </Button>
              </div>
            ) : null}
        ...
        <Card className="shadow-none">
          ...
          <CardContent className="grid gap-4">
            {configuration.groupRoleMappings.map((mapping) => (
              <div className="flex items-center justify-between gap-3 rounded-lg border border-border p-3" key={mapping.id}>
                <span className="truncate text-sm font-medium">{mapping.groupId}</span>
                <Badge variant="outline">{mapping.role}</Badge>
              </div>
            ))}
            <form className="grid gap-3" noValidate onSubmit={(event) => void submitMapping(event)}>
              <ConnectionSelect connections={configuration.connections} disabled={disabled} label={_(COPY.connection)} onChange={setSelectedConnectionId} value={selectedConnectionId} />
              <FieldInput disabled={disabled} id="enterprise-group-id" label={_(COPY.groupId)} onChange={setGroupId} value={groupId} />
              <FieldLabel label={_(COPY.managedRole)}>
                <Select disabled={disabled} onValueChange={(value) => setManagedRole(value as EnterpriseManagedRole)} value={managedRole}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
              ...
              <ActionButton busy={busy === 'mapping'} disabled={disabled} label={_(COPY.saveMapping)} />
            </form>
          </CardContent>
        </Card>
        ...
        <Card className="shadow-none">
          ...
          <CardContent className="grid gap-4">
            {configuration.scimConnections.length ? configuration.scimConnections.map((connection) => (
              <div className="flex items-center justify-between gap-3 rounded-lg border border-border p-3" key={connection.id}>
                <code className="truncate text-xs">{connection.tokenPrefix}…</code>
                <div className="flex items-center gap-2">
                  <Badge variant="outline">{connection.status === 'active' ? _(COPY.active) : _(COPY.statusDisabled)}</Badge>
            ...
            <ConnectionSelect connections={configuration.connections} disabled={disabled} label={_(COPY.connection)} onChange={setSelectedConnectionId} value={selectedConnectionId} />
            <Button disabled={disabled || !selectedConnectionId} onClick={() => void runMutation('scim', async () => {
              const created = await createEnterpriseScimToken(workspaceId, selectedConnectionId);
              setScimToken(created.token);
            })} type="button">
              {busy === 'scim' ? <LoaderCircle aria-hidden="true" className="animate-spin" /> : <KeyRound aria-hidden="true" />}
          ...
          <CardContent className="grid gap-5">
            <form className="grid gap-3" noValidate onSubmit={(event) => {
              ...
              void runMutation('break-glass-request', async () => {
                const created = await createEnterpriseBreakGlassRequest(workspaceId, breakGlassReason.trim());
                setBreakGlassReason('');
                setCreatedBreakGlass({ id: created.id, expiresAt: created.expiresAt });
              });
            }}>
              <FieldInput disabled={disabled} id="enterprise-break-glass-reason" label={_(COPY.reason)} onChange={setBreakGlassReason} value={breakGlassReason} />
              <ActionButton busy={busy === 'break-glass-request'} disabled={disabled} label={_(COPY.requestBreakGlass)} />
            </form>
            {createdBreakGlass ? (
              <Feedback
                kind="success"
            ...
            <form className="grid gap-3" noValidate onSubmit={(event) => {
              ...
              <ActionButton busy={busy === 'break-glass-approve'} disabled={disabled} label={_(COPY.approveBreakGlass)} variant="outline" />
            </form>
          </CardContent>
        </Card>
      </div>
...
function ConnectionList({ connections, disabled, onDisable }: {
  connections: EnterpriseSsoConnection[];
  disabled: boolean;
  onDisable: (connectionId: string) => void;
}): React.ReactElement {
  const { _ } = useLingui();
  if (!connections.length) return <p className="text-sm text-muted-foreground">{_(COPY.noConnections)}</p>;
  return <div className="grid gap-2">{connections.map((connection) => (
    <div className="grid gap-1 rounded-lg border border-border p-3" key={connection.id}>
      <div className="flex items-center justify-between gap-3">
        <span className="truncate text-sm font-semibold">{providerLabel(_, connection.provider)}</span>
        <div className="flex items-center gap-2">
...
function ConnectionSelect({ connections, disabled, label, onChange, value }: {
  connections: EnterpriseSsoConnection[];
  disabled: boolean;
  label: string;
  ...
  return <FieldLabel label={label}>
    <Select disabled={disabled || !connections.length} onValueChange={onChange} value={value}>
      ...
      <SelectContent>{connections.map((connection) => (
        <SelectItem key={connection.id} value={connection.id}>{connection.provider} · {connection.issuer}</SelectItem>
      ))}</SelectContent>
    </Select>
  </FieldLabel>;
}

function FieldInput({ disabled, id, label, onChange, type = 'text', value }: {
  disabled: boolean;
  id: string;
  label: string;
  onChange: (value: string) => void;
  ...
  return <div className="grid gap-2">
    ...
    <Input autoComplete="off" disabled={disabled} id={id} onChange={(event) => onChange(event.target.value)} type={type} value={value} />
  </div>;
}

function FieldLabel({ children, label }: { children: React.ReactNode; label: string }): React.ReactElement {
  return <div className="grid gap-2"><Label>{label}</Label>{children}</div>;
}

function BooleanControl({ checked, disabled, label, onChange }: {
  checked: boolean;
  disabled: boolean;
  label: string;
  onChange: (checked: boolean) => void;
}): React.ReactElement {
  return <label className="flex items-center gap-3 rounded-lg border border-border p-3 text-sm font-medium">
    <input checked={checked} disabled={disabled} onChange={(event) => onChange(event.target.checked)} type="checkbox" />
    {label}
  </label>;
}

function ActionButton({ busy, disabled, label, variant }: {
  busy: boolean;
  disabled: boolean;
  label: string;
  variant?: 'outline';
}): React.ReactElement {
  return <Button disabled={disabled} type="submit" variant={variant}>
...
function ReadonlyValue({ label, value }: { label: string; value: string }): React.ReactElement {
  return <div className="grid gap-1">
    ...
    <code className="overflow-x-auto rounded-md bg-background p-2 text-xs" dir="ltr">{value}</code>
  </div>;
}
