...
export const ACCOUNT_SECURITY_TABS = [
  { id: 'identifier', icon: UserRound, title: COPY.profile, description: COPY.profileHelp },
  { id: 'password', icon: KeyRound, title: COPY.password, description: COPY.passwordHelp },
  { id: 'email', icon: Mail, title: COPY.email, description: COPY.emailHelp },
  { id: 'methods', icon: ShieldCheck, title: COPY.methods, description: COPY.methodsHelp },
  { id: 'sessions', icon: MonitorSmartphone, title: COPY.sessions, description: COPY.sessionsHelp },
  { id: 'data', icon: Download, title: COPY.data, description: COPY.dataHelp },
] as const;

export type AccountSecurityTabId = (typeof ACCOUNT_SECURITY_TABS)[number]['id'];
...
export function AccountSecuritySettings({
  ...
  const [selectedTab, setSelectedTab] = useState<AccountSecurityTabId>('identifier');
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .catch((caught) => {
        if (!active) return;
        statusToast('error', userFacingClientError(caught, _(COPY.unavailable)));
        setBusy('');
      });
    return () => {
      active = false;
  ...
  useEffect(() => {
    const syncTab = (): void => setSelectedTab(accountTabFromHash(window.location.hash));
    syncTab();
    window.addEventListener('hashchange', syncTab);
    return () => window.removeEventListener('hashchange', syncTab);
  }, []);

  function selectTab(tab: AccountSecurityTabId): void {
    setSelectedTab(tab);
  ...
  async function updateUsername(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (!values) return validationFailure(_(COPY.validationRequired), setBusy);
    try {
      setCurrentUsername(await setUsername(values.username, values.currentPassword));
      statusToast('success', _(COPY.saved));
      setBusy('');
      const password = form.elements.namedItem('currentPassword');
  ...
  async function updatePassword(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (
      ...
      return validationFailure(_(COPY.validationPassword), setBusy);
    }
    try {
  ...
  async function updateEmail(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (!values || !/^[^\s@<>]+@[^\s@<>]+\.[^\s@<>]+$/u.test(values.newEmail)) {
      return validationFailure(_(COPY.validationEmail), setBusy);
    }
    try {
      setEmailChange(await startEmailChange(values.newEmail, values.currentPassword));
      statusToast('success', _(COPY.emailQueued));
      setBusy('');
      form.reset();
    } catch (caught) {
      fail(caught);
  ...
  async function removeIdentity(identityId: string, kind: string): Promise<void> {
    ...
    if (kind === 'password' && !password) {
      return validationFailure(_(COPY.validationRequired), setBusy);
    }
    try {
  ...
  async function scheduleDeletion(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (!values || values.confirmation !== 'DELETE') {
      return validationFailure(_(COPY.validationDelete), setBusy);
    }
    try {
  ...
  const activeTab =
    ACCOUNT_SECURITY_TABS.find((tab) => tab.id === selectedTab) ?? ACCOUNT_SECURITY_TABS[0];
  ...
  return (
    <div className="grid gap-8">
      ...
      <div className="grid items-start gap-8 lg:grid-cols-[220px_minmax(0,1fr)]">
        ...
        >
          {ACCOUNT_SECURITY_TABS.map((tab) => {
            const Icon = tab.icon;
            const selected = tab.id === selectedTab;
            return (
              <button
                aria-current={selected ? 'page' : undefined}
                className={`flex min-h-11 shrink-0 items-center gap-3 rounded-lg px-3 text-start text-sm outline-none transition-colors focus-visible:ring-2 focus-visible:ring-ring ${
                  selected
        ...
        <section aria-labelledby="account-tab-heading" className="grid min-w-0 max-w-xl gap-6">
          <div className="grid gap-2">
            <h2 className="text-2xl font-semibold tracking-[-0.025em]" id="account-tab-heading">
              {_(activeTab.title)}
            </h2>
            <p className="text-sm leading-6 text-muted-foreground">{_(activeTab.description)}</p>
          </div>
          ...
          {selectedTab === 'email' ? (
            <div className="grid gap-4">
              {emailChange ? (
                <div className="grid gap-2 rounded-lg border border-border bg-[var(--surface-subtle)] p-3 text-sm">
            ...
            <div className="grid gap-8">
              ...
              <div className="grid gap-4">
                <div className="grid gap-1">
                  <h3 className="text-base font-semibold">{_(COPY.passkeys)}</h3>
                  <p className="text-sm leading-6 text-muted-foreground">{_(COPY.passkeysHelp)}</p>
                </div>
                <ul className="grid gap-2">
              ...
              <div className="grid gap-4">
                <div className="grid gap-1">
                  <h3 className="text-base font-semibold">{_(COPY.recoveryCodes)}</h3>
                  <p className="text-sm leading-6 text-muted-foreground">
                    {_(COPY.recoveryCodesHelp)}
                  </p>
                </div>
                {recoveryStatus ? (
          ...
          {selectedTab === 'data' ? (
            <div className="grid gap-8">
              ...
              <div className="grid gap-4 border-t border-[var(--danger-border)] pt-6">
                <div className="grid gap-1 text-[var(--danger-fg)]">
                  <h3 className="flex items-center gap-2 text-base font-semibold">
                    <Trash2 aria-hidden="true" className="size-4" />
                    {_(COPY.danger)}
                  </h3>
                  <p className="text-sm leading-6 text-[var(--danger-body)]">{_(COPY.dangerHelp)}</p>
                </div>
                <form
                  className="grid gap-3"
                  noValidate
...
function validationFailure(message: string, setBusy: (value: string) => void): void {
  statusToast('error', message);
  setBusy('');
}
...
function accountTabFromHash(hash: string): AccountSecurityTabId {
  const value = hash.replace(/^#/, '');
  return ACCOUNT_SECURITY_TABS.some((tab) => tab.id === value)
    ? (value as AccountSecurityTabId)
    : 'identifier';
}
