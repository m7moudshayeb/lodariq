...
import { AUTH_SHELL_MESSAGES } from '../i18n/messages';
...
export async function AuthShell({
  ...
  children,
  backHref,
}: AuthShellProps): Promise<React.ReactElement> {
        ...
        <section className="relative hidden flex-col justify-between overflow-hidden border-e border-border bg-[var(--surface-subtle)] p-10 lg:flex">
          ...
          <div className="grid max-w-sm gap-5 pb-8">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-primary">
              {i18n._(AUTH_SHELL_MESSAGES.authorOnce)}
            </p>
            <p className="[font-family:Georgia,serif] text-[2.65rem] leading-[1.08] tracking-[-0.03em] text-foreground">
              {i18n._(AUTH_SHELL_MESSAGES.productExperiences)}
            </p>
            <p className="max-w-xs text-sm leading-6 text-muted-foreground">
              {i18n._(AUTH_SHELL_MESSAGES.workflow)}
            </p>
          </div>
          <p className="text-xs text-muted-foreground">
            {i18n._(AUTH_SHELL_MESSAGES.secureAccess)}
          </p>
        </section>
              ...
              <div className="flex items-center justify-between lg:hidden">
                ...
                {backHref ? (
                  ...
                  >
                    ...
                    {i18n._(AUTH_SHELL_MESSAGES.back)}
                  </Link>
                ) : null}
              </div>
