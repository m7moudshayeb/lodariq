  ...
  async activateCompiledArtifact(
    ...
    const outcome = await this.actorScoped(
      ...
      async (tx): Promise<ReleaseOutcome> => {
        ...
        await tx.execute(
          sql`select pg_advisory_xact_lock(hashtextextended(${`live-stock:${input.workspaceId}`}, 0))`,
        );
        ...
        if (liveLimit !== null) {
          const [stock, existing] = await Promise.all([
            ...
            tx
              ...
              .where(
                and(
                  eq(documentDeployments.workspaceId, input.workspaceId),
                  eq(documentDeployments.documentId, artifact.documentId),
                  eq(documentDeployments.state, 'active'),
                ),
              )
              .limit(1),
          ...
          if (!existing[0] && used + 1 > liveLimit) {
            throw new CommercialEntitlementError('live-experiences', used, liveLimit);
          }
        }
