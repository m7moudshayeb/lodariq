...
describe('unified popup content canvas', () => {
  ...
  it('authors continuous rich content while the popup remains output-only', async () => {
    ...
    expect(document.querySelector('.rich-content-selection-trigger')).toBeNull();
    ...
    await vi.waitFor(() =>
      expect(canvas.querySelector('[aria-label="Button label"]')).toBeInstanceOf(HTMLInputElement),
    );
    expect(canvas.querySelector('[aria-label="After click"]')).toBeInstanceOf(HTMLButtonElement);
    ...
    await vi.waitFor(() => {
      const authoredMedia = document.querySelector<HTMLElement>(
        '.rich-step-media-preview:not(.placeholder)',
      );
      const authoredImage = authoredMedia?.querySelector<HTMLImageElement>('img');
      expect(authoredMedia?.style.width).toBe('95%');
      expect(authoredImage?.style.height).toBe('64px');
      expect(authoredImage?.style.maxHeight).toBe('none');
      expect(authoredMedia?.querySelector('figcaption')).toBeNull();
    });
