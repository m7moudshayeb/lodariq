...
- Current phase: **Pre-Phase — Full Local SDK Foundation** (PRD §16.1)
...
| Playwright (PRD §16.0)                                           | ⏳     | Pre-phase e2e still pending (PRD §16.1)               |
...
| ----------------------------------------------------------------------------- | ------ | ------------------------------------------------------------------------------------------ |
| CI runs typecheck, lint, tests, and bundle-size checks                        | 🟡     | Workflow exists; `pnpm verify` includes size, full re-run still pending after latest edits |
...
| Runtime lifecycle hints (PRD §8.6)                           | 🟡     | Route/text/element waits and scroll-container handling; virtualized-list behavior still pending                   |
...
| Bridge message protocol + envelope (PRD §9.5, §11.1)         | ✅     | `schema/src/bridge.ts`, validated in `sdk-authoring/src/bridge`, ack/timeouts included                            |
...
- **Pre-phase remaining (PRD §16.1):** full Lexical MVP nodes (paragraph,
  heading, tour step, tooltip, button, target chip, validation badge), block
  transforms, drag/drop + keyboard reorder, Lexical-state serialization,
  clipboard coverage in the Lexical path, Playwright e2e against the fixture
  host, and virtualized-list lifecycle behavior.
